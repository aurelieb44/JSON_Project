...
for univ in list_of_univ:
    ...
    if conf == 372 or conf == 108 or conf == 107 or conf == 127 or conf == 130 :
        """
        try:
            price = int(univ["Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)"])
        
        except ValueError:  
            instit = univ["instnm"]
            print(f"Missing Data for {instit}") 

        else:
        """

            #if int(univ["Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)"])>50000:
        if 0 if univ["Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)"] is None else univ["Total price for in-state students living off campus (not with family)  2020-21 (DRVIC2020)"]>50000:
            try:
...
my_layout = Layout(title='Price for in-state students living off campus for Big XII Universities')
